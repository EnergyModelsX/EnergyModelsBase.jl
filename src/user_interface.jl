...
function read_data(fn)
    ...
    nodes = [
            ...
            RefSource(2, FixedProfile(1e12), FixedProfile(30), FixedProfile(0), Dict(NG => 1), 𝒫ᵉᵐ₀,Dict("InvestmentModels" => data_test())),  
            RefSource(3, FixedProfile(1e12), FixedProfile(9), FixedProfile(0), Dict(Coal => 1), 𝒫ᵉᵐ₀,Dict("InvestmentModels" => data_test())),  
            RefGeneration(4, FixedProfile(25), FixedProfile(5.5), FixedProfile(0), Dict(NG => 2), Dict(Power => 1, CO2 => 1), 𝒫ᵉᵐ₀, 0.9,Dict("InvestmentModels" => data_test())),  
            RefGeneration(5, FixedProfile(25), FixedProfile(6), FixedProfile(0),  Dict(Coal => 2.5), Dict(Power => 1, CO2 => 1), 𝒫ᵉᵐ₀, 0,Dict("InvestmentModels" => data_test())),  
            RefStorage(6, FixedProfile(60), FixedProfile(600), FixedProfile(9.1), FixedProfile(0),  Dict(CO2 => 1, Power => 0.02), Dict(CO2 => 1),Dict("InvestmentModels" => data_test())),
